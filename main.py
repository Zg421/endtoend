    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        with tqdm(dataloader.data, unit='batch') as tepoch:
            for sample_batched in tepoch:
                ...
                edge_curvature_val = edge_curvature_loss_fn(depth_est)
                loss_edge_curvature = edge_curvature_val * args.w_edge_curvature

                gradient_aniso_val = gradient_aniso_loss_fn(depth_est)
                loss_gradient_aniso = gradient_aniso_val * args.w_gradient_aniso
                ...
                tepoch.set_postfix(loss=loss.item(),
                                   ...
                                   l_edge=loss_edge_curvature.detach().item(),
                                   l_aniso=loss_gradient_aniso.detach().item(),
                                   edge_raw=edge_curvature_val.detach().item(),
                                   aniso_raw=gradient_aniso_val.detach().item())
